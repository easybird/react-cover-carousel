...
export default function withScrollTresholdIndication (
  ...
  return class extends React.Component {
    ...
    resetScrollingTimeout = () => {
      ...
      this.scrollingTimeout = setTimeout (this.handleResetScrolling, this.resetScrollTimeoutMs);
    };
    ...
    updateScrollingDistance = e => {
      ...
      if (this.treshold > absValue) {
        ...
        const roundedTresholdPercentage = Math.floor (tresholdPercentage * 10) / 10;
        if (roundedTresholdPercentage !== this.state.tresholdPercentage) {
        this.debug('--setState tresholdPercentage', roundedTresholdPercentage);
